...
#' @param s2s.units: The number of encoded units in S2S architecture.
#' @param s2s.dropout: Fraction of encoded S2S context to drop.
#' @param ae.depth: The number of S2S autoencoder layers.
#' @param ae.middle: The number of neurons in the middle of S2S autoencoder.
#' @param ae.dropout: Fraction of neurons to drop from S2S autoencoder.
...
DCL = function(x.count,
               ...
               cnn.dropout = 0.5,
               cnn.type = "avg",
               s2s.units = 0.7*lookback,
               s2s.dropout = 0,
               ae.depth = 3,
               ae.middle = 0.6*s2s.units,
               ae.dropout = 0.5) {
    ...
    y = y.past %>%
        ...
        k_S2S(x, "LSTM", s2s.units, s2s.dropout, ae.depth, ae.middle, ae.dropout) %>%
...
LoadDNN = function(folder, file, type = "DCL") {
    ...
    if (type == "DeepAR") return(g_Load(folder))
