...
#' k_Autoencoder: keras-based fully-connected autoencoder.
#' @param input: Previous layer.
...
k_Autoencoder = function(input, depth, edge, middle = edge/3, dropout = 0.5, name = "AE") {
...
#' k_S2S: keras-based bidirectional Sequence-to-Sequence (S2S) architecture with attention mechanism.
...
#' @param units: The number of encoded units.
#' @param dropout: Fraction of encoded units to drop.
#' @param ae.depth: The depth of autoencoder between S2S encoder and decoder.
#' @param ae.middle: The number of neurons in the middle of the autoencoder.
#' @param ae.dropout: Fraction of neurons to drop from the autoencoder.
#' @param name: Layer name.
#' @return S2S layers.
k_S2S = function(past, future, type = "LSTM", units, dropout = 0, ae.depth = 0, ae.middle = units/3, ae.dropout = 0.5, name = "S2S") {
        ...
        EncodedStates = function() {
            GRUStates = function() {
                h = layer_concatenate(encoded[2:3], name = "hidden") %>%
                    ...
                    k_Autoencoder(ae.depth, units, ae.middle, ae.dropout, name = k_Name(name, "hidden_AE"))
                return(h)
            }
            LSTMStates = function() {
                ...
                c = layer_concatenate(encoded[c(3, 5)], name = k_Name(name, "cell")) %>%
                    ...
                    k_Autoencoder(ae.depth, units, ae.middle, ae.dropout, name = k_Name(name, "cell_AE"))
                return(list(h, c))
            }
