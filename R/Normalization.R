...
NormParam = function(data, type = "Z", by = "") {
    AbsMax = function(values) return(c(absmax = max(abs(values))))
    MinMax = function(values) return(c(min = min(values), max = max(values)))
    Avg = function(values) return(c(avg = mean(values)))
    Z = function(values) return(c(avg = mean(values), stdev = sd(values)))
