...
#' Filtering is based on Joint Mutual Information Maximisation (JMIM) and elbow method.
...
SelectFeatures = function(x, y, focus = 1:Count(x)) {
    Score = function() {
        ...
        score = JMIM(fx, unlist(fy), ncol(fx))$score
        return(sort(score, decreasing = TRUE))
    }
    Elbow = function(score) {
        diff = score-c(score[2:Count(score)], 0)
        arm = diff==max(diff)
        for(i in 1:Count(arm)) {
            if (arm[i]) break
            arm[i] = TRUE
        }
        return(score[arm])
